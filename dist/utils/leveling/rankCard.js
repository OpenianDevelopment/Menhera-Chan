...
class RankCard {
    constructor() {
        this.data = {
            width: 1026,
            height: 285,
            backgroundImage: "https://cdn.discordapp.com/attachments/791301593391562752/856879146175954954/rankcard2.png",
            color: {
                main: "#f5deb3",
                track: "#21cc87",
            },
        ...
        const familyName = ["Dela_Gothic_One", "GothicA1"];
        for (const family of familyName)
            fs_1.readdirSync(__dirname + `/fonts/${family}`)
                .filter((file) => file.endsWith(".ttf"))
                .forEach((file) => {
                canvas_1.registerFont(__dirname + `/fonts/${family}/${file}`, {
                    family: `${family}`,
                });
            });
    }
    ...
    build() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const MaxXp = toAbbrev(this.data.MaxXP), Xp = toAbbrev(this.data.xp), rank = toAbbrev(this.data.rank);
            let bg = yield canvas_1.loadImage(this.data.backgroundImage);
            let avatar = yield canvas_1.loadImage(this.data.avatarURL);
            // create canvas instance
            const canvas = canvas_1.createCanvas(this.data.width, this.data.height);
            ...
            roundRect(ctx, canvas.width - 330 + 62.5, 75.5 /*(- 41)*/, 205, 32.5 /* (* 2) */, 7); //* level in box
            roundRect(ctx, canvas.width - 330 + 62.5, 195.5 /*(- 41)*/, 205, 32.5 /* (* 2) */, 7); //* rank in box
            // reset transparency
            ctx.globalAlpha = 1;
            // draw username
            ...
            ctx.fillStyle = color.main;
            ctx.textAlign = "start";
            ctx.fillText(Xp + " / " + MaxXp, 380 + ctx.measureText(Xp.toString()).width + 25, 251);
            // draw progress-track
            ctx.fillStyle = "black";
            ctx.fillRect(0, 270, 1026, 20);
            // progress-bar
            ctx.fillStyle = color.track;
            ctx.fillRect(0, 270, ((this.data.xp - this.data.MinXP) /
                (this.data.MaxXP - this.data.MinXP)) *
                1026, 20);
            ctx.arc(((this.data.xp - this.data.MinXP) /
                (this.data.MaxXP - this.data.MinXP)) *
                1026, 20, 8, 0.5 * Math.PI, 1.5 * Math.PI, true);
            ctx.save();
            // Avatar circle
            ctx.beginPath();
            ...
            ctx.restore();
            //return the card
            return new discord_js_1.MessageAttachment(canvas.toBuffer(), `${this.data.username}.png`);
        });
    }
}
