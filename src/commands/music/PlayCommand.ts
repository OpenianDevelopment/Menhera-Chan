...
export default class PlayCommand extends BaseCommand {
	...
	async run(client: DiscordClient, message: Message, args: Array<string>) {
		...
		if (!permissions?.has("CONNECT")) {
			const embed = new MessageEmbed()
				.setDescription("I don't have permission to `Connect` in that channel.")
				.setColor("RED");
			message.reply({ embeds: [embed] });
			return;
		}
		if (!permissions.has("SPEAK")) {
			const embed = new MessageEmbed()
				.setDescription("I don't have permission to `Speak` in that channel.")
				.setColor("RED");
			message.reply({ embeds: [embed] });
			return;
		}
		...
		if (SpotifyTrackPattern.test(query)) {
			PlaySpotifyTrack(client, query, message);
			return;
		} else if (SpotifyPlaylistPattern.test(query)) {
			GetSpotifyPlaylistTracks(client, query, message);
			return;
		}
