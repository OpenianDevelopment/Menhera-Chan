...
module.exports = {
    ...
    interactionMsg: async (client, args, embed, content, mentionedUsers) => {
        const data = {
            "type": 4,
            "data": {
                "tts": false,
                "content": content,
                "embeds": [embed],
                "allowed_mentions": mentionedUsers
            }
        }
    ...
    addStarMessage: async function (client, message, stars) {
        ...
        addDBStarMessage(message, TheMsg, stars);
        return;
    },
    editStarMessage: async function (message, newstars) {
        const starData = await getStarboard(message.guild.id);
        if (!starData) return;
        const postedStarMsg = starData.messages.find(d => d.posted_message === message.id || d.message === message.id);
        if (!postedStarMsg) return;
        await editStarCount(message, newstars)
        message.channel.messages.fetch(postedStarMsg.posted_message).then(async msg => {
            if (msg.author.id === message.guild.me.user.id) {
                /*msg.react('⭐').then(
                    msg.react('801787425156235275')
                )*/
                await msg.edit(`**${newstars} ⭐**`);
            }
        })
    },
    deleteStarMessage: async function (message) {
        const starData = await getStarboard(message.guild.id);
        if (!starData) return;
        const PostedStarMsg = starData.messages.find(c => c.message === message.id || c.posted_message === message.id);
        message.channel.messages.fetch(PostedStarMsg.posted_message).then(async msg => {
            if (msg.author.id === message.guild.me.user.id) {
                await msg.delete();
                deleteDBStarMessage(message);
            }
        })
        return;
    },
