...
const starboard = require('../database/starboard');
module.exports = {
    ...
    addWaifu: async function (N, I, G, A, C) {
        var data = await waifu.find()
    ...
    removeWaifu: async function (ID) {
        waifu.findOneAndDelete({ id: ID }, function (err, result) {
            if (err) throw err
            if (err) return false
        })
        return
    ...
    initStarboard: function (guildID, Post_channel) {
        starboard.findOneAndDelete({}).findOne({
            guild: guildID
        }, (err, res) => {
            if (err) throw err;
            if (!res) {
                var NewStarBoard = new starboard({
                    guild: guildID,
    ...
    getStarboard: async function (guildID) {
        const starData = await starboard.findOne({ guild: guildID });
        if (!starData) return;
        return starData;
    },
    addDBStarMessage: async function (message, posted_msg) {
        starboard.findOne({ guild: message.guild.id }),
            (err, res) => {
                if (err) throw err;
                if (res) {
                    res.messages.push({
                        ...
                        message: message.id,
                        posted_message: posted_msg.id
                    })
                    res.save()
                }
            }
    },
    deleteDBStarMessage: async function (message) {
        starboard.findOne({ guild: message.guild.id }),
            (err, res) => {
                if (err) throw err;
                if (res) {
                    return res.messages.findOneAndDelete({ message: message.id })
                }
            }
    },
}