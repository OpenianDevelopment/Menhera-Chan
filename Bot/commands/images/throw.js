...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        //if(message.guild.id==="735899211677041099"&&member.id==="534783899331461123") return message.channel.send("You dare think about throwing my master!!!").then(message.channel.stopTyping(true));
        //if(message.guild.id==="735899211677041099"&&member.id==="722510934932848812"&&message.author.id!=="534783899331461123") return message.channel.send("Wait that's my sister, You want to throw her!! Consider yourself dead!!!").then(message.channel.stopTyping(true));
        ...
        const createdAt = moment.duration(Date.now() - member.user.createdTimestamp).format(`M`); 
        //the member's account age in months ^
        ctx.font = applyText(canvas, createdAt);
        ctx.fillStyle = '#040404';
        ctx.fillText(createdAt, 145, 96, 160); //writing the months
